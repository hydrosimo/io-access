...
open(2239,file='ddt_pos.dat',access='direct',status='unknown',form='unformatted',recl=nbrl)
open(1875,file='stream.dat',access='stream',status='unknown',form='unformatted')
...
! do i=n_el,1,-1
    ...
    read(1875,pos=dr_pt%pos_cum) dr_pt_vl%id_pnts
    
    ! WRITE(6,*) ' '
    ! WRITE(6,*) '--------------------'
    ! WRITE(6,*) 'dr_pt%pos_cum = ',dr_pt%pos_cum
    ! WRITE(6,*) 'dr_pt_vl%id_pnts = ',dr_pt_vl%id_pnts
